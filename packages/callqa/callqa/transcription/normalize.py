...
def _echo_phrases(cfg: dict) -> list[str]:
    ...
    # Site-specific hold/IVR phrases (client-specific — set locally, not committed)
    phrases.update(cfg.get("extra_echo_phrases") or [])
    return sorted((p for p in phrases if p), key=len, reverse=True)


def dedupe_hold_phrases(text: str, cfg: dict | None = None) -> str:
    ...
    # Site-specific IVR/hold patterns (client-specific — set locally, not committed)
    patterns.extend((cfg or {}).get("extra_hold_patterns") or [])
    result = text
    for pat in patterns:
